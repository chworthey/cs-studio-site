...
*Content Privacy Checks:* I'm giving myself a pat on the back for this one. Photos, particularly ones taken with a cell phone, may have GPS coordinates embedded into the EXIF metadata. Most engines would simply ignore the metadata, but this would mean that if one were to check the source image into git on a publicly-hosted repository, you can effectively dox yourself, even if the photo is deleted in a git commit afterwards. Applications can also insert who-knows-what into the metadata, further putting privacy at risk. Tofu engine refuses to build your project if it detects any sort of metadata on images. Students must strip any metadata before proceeding, a habit that once formed will keep them safe their entire lives. It is so important to protect our students from this particular privacy issue that has led to human deaths! And yet it's a privacy issue that few know about. If I were to expand on this check in the future, I would probably enforce an extended version of git to prevent people from checking in image metadata, and then add an online repository scanner routine automation just in case.
