...
import { IVirtualKeyboard, MaskModeType, VirtualKeyboardAction, VirtualKeyboardPage, KeysByAlpha, KeysByNum, KeysBySymbol } from '../types/VirtualKeyboardTypes';
...
export function VirtualKeyboard(props: IVirtualKeyboardProps) {
  ...
  let maskKeys = new Set<KeyboardKey>(config.KeyMask?.Keys);
  const maskActions = new Set(config.ActionMask?.Actions);

  const keyAllowed = (key: KeyboardKey) => {
    let rv = true;
    if (config.KeyMask) {
  ...
  const alphaPage = <div className='div__keyboard-col'>
    <div className="div__keyboard-row">
      <Key Content="TAB" KeySize={keySize} WidthScale={tabKeyWidthScale} OnClick={() => {
        ...
        newKeyboard.OnKey(KeyboardKey.Tab);
        onUpdate(newKeyboard);
      }} IsEnabled={keyAllowed(KeyboardKey.Tab)}/>
      {alphaHigh.map(a => <Key
        key={a}
    ...
    <div className="div__keyboard-row">
      <Key Content="CAPS" KeySize={keySize} WidthScale={capsKeyWidthScale} IsActive={keyboard.IsCapsOn} OnClick={() => {
        ...
        newKeyboard.OnKey(KeyboardKey.Caps);
        onUpdate(newKeyboard);
      }} IsEnabled={keyAllowed(KeyboardKey.Caps)}/>
      {alphaMid.map(a => <Key
        key={a}
        Content={capitalized ? a.toUpperCase() : a}
        KeySize={keySize}
    ...
    <div className="div__keyboard-row">
      <Key Content="SHIFT" KeySize={keySize} WidthScale={shiftKeyWidthScale} IsActive={keyboard.IsShiftOn} OnClick={() => {
          ...
          onUpdate(newKeyboard);
        }} IsEnabled={keyAllowed(KeyboardKey.Shift)}/>
      {alphaLow.map(a => <Key
        key={a}
        Content={capitalized ? a.toUpperCase() : a}
        KeySize={keySize}
        ...
        <div className='div__keyboard-col'>
          <div className="div__keyboard-row">
            <Key Content="INS" KeySize={keySize} IsActive={keyboard.IsInsertOn} OnClick={() => {
                ...
                newKeyboard.OnKey(KeyboardKey.Insert);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Insert)}/>
            <Key Content="HM" KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.Home);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Home)}/>
            <Key Content="END" KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.End);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.End)}/>
          </div>
          <div className="div__keyboard-row">
            <Key Content="PU" KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.PageUp);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.PageUp)}/>
            <Key Content={"\u{25B2}"} KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.Up);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Up)}/>
            <Key Content="PD" KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.PageDown);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.PageDown)}/>
          </div>
          <div className="div__keyboard-row">
            <Key Content={"\u{25C4}"} KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.Left);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Left)}/>
            <Key Content={"\u{25BC}"} KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.Down);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Down)}/>
            <Key Content={"\u{25BA}"} KeySize={keySize} OnClick={() => {
                const newKeyboard = keyboard.Clone();
                newKeyboard.OnKey(KeyboardKey.Right);
                onUpdate(newKeyboard);
              }} IsEnabled={keyAllowed(KeyboardKey.Right)}/>
          </div>
        </div>
      </div>
      <div className="div__keyboard-row">
        ...
        <Key Content="SPACE" KeySize={keySize} WidthScale={spaceKeyWidthScale} AdditionalClass="div__space-key"
          OnClick={() => {
              const newKeyboard = keyboard.Clone();
              newKeyboard.OnKey(KeyboardKey.Space);
              onUpdate(newKeyboard);
            }} IsEnabled={keyAllowed(KeyboardKey.Space)}/>
        <Key Content="BACKSPACE" KeySize={keySize} WidthScale={backspaceKeyWidthScale}
          OnClick={() => {
              const newKeyboard = keyboard.Clone();
              newKeyboard.OnKey(KeyboardKey.Backspace);
              onUpdate(newKeyboard);
            }} IsEnabled={keyAllowed(KeyboardKey.Backspace)}/>
        <Key Content="DEL" KeySize={keySize}
          OnClick={() => {
              const newKeyboard = keyboard.Clone();
              newKeyboard.OnKey(KeyboardKey.Delete);
              onUpdate(newKeyboard);
            }} IsEnabled={keyAllowed(KeyboardKey.Delete)}/>
        <Key Content="ENTER" KeySize={keySize} WidthScale={enterKeyWidthScale}
          OnClick={() => {
              const newKeyboard = keyboard.Clone();
              newKeyboard.OnKey(KeyboardKey.Enter);
              onUpdate(newKeyboard);
            }} IsEnabled={keyAllowed(KeyboardKey.Enter)}/>
      </div>
    </div>
  );
}